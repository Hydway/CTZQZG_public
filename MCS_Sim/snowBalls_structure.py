...
class Structure:
    ...
    def CLA_call_boost(self, obsv_list: list):
        # is_fix 产品是否固定
        # length_boost 指增长度
        # a_boost 指增超额收益
        i = self.i
        ...
        for obsv in obsv_list:
            if self.data['close'][obsv] >= self.CLOSE * self.KO:
                ...
                # 曾敲入
                    ...
                    # 加期权费
                    res['profit'] = (self.coupon_monthly * (self.freez + obsv_list.index(obsv) + 1) + (
                                self.data['close'][obsv] - self.CLOSE * self.KI) / (
        ...
        if price_min < self.CLOSE * self.KI:
            if self.data['close'][last_date] < self.CLOSE:
                ...
                if self.data['close'][last_date] < self.CLOSE * self.KI:
                    res['date'] = self.data['Date'][last_date]
                    res['FLAG'] = -1
                    res['profit'] = ((self.data['close'][last_date] - self.CLOSE) / self.CLOSE - self.call_cost) / (
                                1 + self.call_cost)
                    res['out_price'] = self.data['close'][last_date]
                    res['lasting'] = self.freez + len(obsv_list)
                    res['+call'] = 1
                ...
                # 末期稳定
                    ...
                    res['date'] = self.data['Date'][last_date]
                    res['FLAG'] = -1
                    res['profit'] = ((self.data['close'][last_date] - self.CLOSE) / self.CLOSE + (
                                self.data['close'][last_date] - self.CLOSE * self.KI) / (
                                                 self.CLOSE * self.KI) - self.call_cost) / (1 + self.call_cost)
                    res['out_price'] = self.data['close'][last_date]
                    res['lasting'] = self.freez + len(obsv_list)
                    res['+call'] = 1
        ...
        # 指增
        date_in = last_date
        end_date = date_in + (self.length_boost-1) * 21
